...
(defvar sql-mysql-databases nil
  "The databases will build `sql-mysql-schema'.
If you set this variable non nil, the sql-mysql-schema will order as this.")
...
(defun sql-set-params-from-sql-connect ()
  "Set completion params from `sql-connect`."
  (if (boundp 'sql-product)
      (progn
	(setq mysql-host sql-server)
	(setq mysql-user sql-user)
	(setq mysql-password sql-password)
	(setq sql-mysql-database sql-database)
	(if (and sql-mysql-databases (not (member sql-mysql-database sql-mysql-databases)))
	    (setq sql-mysql-databases nil)
	    ))))

(defun sql-mysql-completion-init ()
  "Initialize completetions lists."
  (interactive)
  (sql-set-params-from-sql-connect)
...
(defun sql-mysql-build-databases (&optional rebuild)
  "Build database struct if empty or REBUILD passed."
  (if (or rebuild
...
(defun sql-mysql-update-database (db)
  "If you make some change in certain DB, you can update `sql-mysql-schema' use this command."
  (interactive (list
                (progn
...
(defun sql-mysql-refresh-schema ()
  "Refresh schema of all databases.
Be careful, this could be very slow."
  (interactive)
...
(defun sql-mysql-input-filter (sql)
  "When use SQL \"use dbname\" to change database, change variable sql-mysql-database too."
