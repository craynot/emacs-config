...
(defun mysql-filter (proc string)
  (when (string-match mysql-prompt-regexp string)
...
(defun mysql-query (sql proc)
  "Query SQL in PROC until get result.
But if the time which mysql takes to process
...
If you are sure their is output, set WAIT non-nil"
  ...
  (message "3")
  (setq cnt 10)
  (while (and (not (mysql-finished proc)) (> cnt 0))
	(setq cnt (- cnt 1))
    (sit-for 0.1))
  (message "4")
...
(defun mysql-shell-query (sql &optional db)
  (let ((cmd (mapconcat
	       ...
	       (and (string< "" mysql-password)
		    (list (concat "MYSQL_PWD=" mysql-password)))

	       (append (list mysql-program)
                              ;; -s option inhibit header in output
                              (remove "-s" mysql-options))
                      (list
                       "-u" mysql-user
                       db
                       (and (string< "" mysql-host)
                            (concat "-h" mysql-host))
                       "-e" (format "\"%s\"" sql)))
              " ")))
